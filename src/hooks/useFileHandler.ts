...
import { FilePreviewItem, UploadedFileInfo } from "@/types";
...
export const useFileHandler = () => {
  ...
  const processFiles = useCallback(
    (files: FileList | null) => {
      ...
      const uniqueNewFiles = newUploadedFilesArray.filter(
  ...
  const handleRemovePreview = useCallback(
    (fileIdToRemove: string) => {
      const removedPreview = filePreviews.find(
      ...
      if (removedPreview) {
        setDroppedFiles((prev) =>
          prev.filter((file) => file.name !== removedPreview.name),
        );
      }
    },
    [filePreviews],
